...
/**
 * returnTo を扱うための共通 Hook。
 *
 * 用途:
 * - 現在画面から次画面への href を returnTo 付きで作る
 * - 現在 URL に含まれている returnTo を安全に読み取る
 *
 * 主な利用場面:
 * - 一覧 → 詳細リンクを作る
 * - 詳細画面で戻る先 URL を取得する
 *
 * 返すもの:
 * - createHref: 遷移先 href を作る関数
 * - returnTo: 現在 URL に含まれる安全な戻り先。無ければ undefined
 *
 * @example
 * const { createHref, returnTo } = useReturnTo()
 *
 * const detailHref = createHref('/detail', { id: 'A' })
 * // '/detail?id=A&returnTo=...'
 *
 * console.log(returnTo)
 * // '/results?keyword=apple&page=2' など
 */
export function useNavigationHref() {
   ...
   * // '/detail?id=A&returnTo=%2Fresults%3Fkeyword%3Dapple'
   */
  const createHref = (
    nextPathname: string,
  ...
  const safeReturnTo = isSafeInternalReturnTo(returnTo) ? returnTo : undefined
